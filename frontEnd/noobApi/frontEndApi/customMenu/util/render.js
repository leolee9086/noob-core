...
function 生成子菜单(目标菜单项元素, 子菜单项配置) {
  let 子菜单容器 = 目标菜单项元素.querySelector(".b3-menu__submenu");
  if (!子菜单容器) {
    目标菜单项元素.insertAdjacentHTML(
      "beforeend",
      `<div class="b3-menu__submenu"></div>`
    );
    子菜单容器 = 目标菜单项元素.querySelector(".b3-menu__submenu");
  }
  (子菜单项配置) => {
    try {
      子菜单容器.appendChild(生成多级菜单项目元素(子菜单项配置));
