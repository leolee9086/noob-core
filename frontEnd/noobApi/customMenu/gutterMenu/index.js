  ...
  if (判定元素&&判定元素.parentElement&&判定元素!==判定元素.parentElement) {
    switch (判定元素.tagName) {
      ...
      case "svg":
        return 判断是否块标菜单(判定元素.parentElement);
      case "BUTTON":
        if (判定元素.getAttribute("data-node-id")) {
          return 判断是否块标菜单(判定元素.parentElement);
        } else {
          return false;
        }
      case "DIV":
        if (
          判定元素.classList &&
          判定元素.classList.contains("protyle-gutters")
        ) {
          块标菜单.菜单状态.当前块id = 判定元素
            .querySelector("BUTTON")
            .getAttribute("data-node-id");
          块标菜单.菜单状态.当前块类型 = 判定元素
            .querySelector("BUTTON")
            .getAttribute("data-type");
          块标菜单.菜单状态.当前子块类型 = 判定元素
            .querySelector("BUTTON")
            .getAttribute("data-subType");
        }
        return (
          判定元素.classList && 判定元素.classList.contains("protyle-gutters")
        );
    }
  }
}
