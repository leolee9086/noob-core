...
import addEsmSurrport from '../noob-service-compiler/esm/esmProxy.js'
import EventBridge from 'http://127.0.0.1:6806/appearance/themes/noob/script/messageBridge/index.js'
let event_bridge= new EventBridge("noob-service-sypublisher","noob-service-sypublisher")
event_bridge.handler("设置首页",(data)=>{
    event_bridge.invoke("noobSyPublisherMain","设置首页成功",{id:data.id})
})
            ...
            doc => {
                fetch(`http://127.0.0.1/block?id=${doc.id}&&clear=true`).then(
                    async() => { 
                       ...
                       event_bridge.call("siyuanMain","status__msg",{ msg: `${doc.id}发布缓存刷新` })
                    }
                )
            }
        )
    })
    document.querySelector("#preViewer").setAttribute('src',`http://127.0.0.1/block?id=${设置.首页}&&clear=true`)
