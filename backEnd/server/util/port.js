...
export function 写入端口记录(记录名,端口号){
    let 端口记录文件名 = toAbsolute(portPath)
...
export let noob依赖服务端口号 =await 获取可用端口号(noob核心后端服务端口号+1) 
export let noob编译服务端口号 =await 获取可用端口号(noob依赖服务端口号+1) 

export function 初始化端口文件(){
    写入端口记录('noob核心后端服务端口号',noob核心后端服务端口号)
    写入端口记录('思源核心服务端口号',思源核心服务端口号)
    写入端口记录('noob依赖服务端口号',noob依赖服务端口号)
    写入端口记录('noob编译服务端口号',noob编译服务端口号)
